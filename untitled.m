...
% %         Jnim1by2 = (q*mun(2)*Vt/dx*Ldi) * ( (n(2)*ni)*BER((fi(2)-fi(2-1)))      - (n(2-1)*ni)*BER((fi(2-1)-fi(2))) );
% %         Jnip1by2 = (q*mun(2)*Vt/dx*Ldi) * ( (n(2+1)*ni)*BER((fi(2+1)-fi(2)))    - (n(2)*ni)*BER((fi(2)-fi(2+1))) );
% %         Jelec(vindex) = (Jnip1by2 + Jnim1by2)/2;

          % Hole Current   
% %         Jpim1by2 = (q*mup(2)*Vt/dx*Ldi) * ( (p(2)*ni)*BER((fi(2-1)-fi(2)))         - (p(2-1)*ni)*BER((fi(2)-fi(2-1))) );
% %         Jpip1by2 = (q*mup(2)*Vt/dx*Ldi) * ( (p(2+1)*ni)*BER((fi(2)-fi(2+1)))       - (p(2)*ni)*BER((fi(2+1)-fi(2))) );
